...
def get_location(insert=''):
	return 'https://github.com/%s/%s/raw/main/packages/%s' % (get_setting('redlight.update.username'), get_setting('redlight.update.location'), insert)

def get_versions():
	try:
		result = requests.get(get_location('redlightam_version'))
...
def get_changes(online_version=None):
	try:
		if not online_version:
			...
			if not version_check(current_version, online_version): return kodi_utils.ok_dialog(heading='Red Light Updater',
				text='You are running the current version of Red Light.[CR][CR]There is no new version changelog to view.')
		kodi_utils.show_busy_dialog()
		result = requests.get(get_location('redlightam_changes'))
...
def update_check(action=4):
	...
	if not version_check(current_version, online_version):
		if action == 4: return kodi_utils.ok_dialog(heading='Red Light Updater', text='Installed Version: [B]%s[/B][CR]Online Version: [B]%s[/B][CR][CR] %s' \
			% (current_version, online_version, '[B]No Update Available[/B]'))
		return
	if action in (0, 4):
		if not kodi_utils.confirm_dialog(heading='Red Light Updater', text='Installed Version: [B]%s[/B][CR]Online Version: [B]%s[/B][CR][CR] %s' \
			% (current_version, online_version, '[B]An Update is Available[/B][CR]Perform Update?'), ok_label='Yes', cancel_label='No'): return
		if kodi_utils.confirm_dialog(heading='Red Light Updater', text='Do you want to view the changelog for the new release before installing?', ok_label='Yes', cancel_label='No'):
			get_changes(online_version)
			if not kodi_utils.confirm_dialog(heading='Red Light Updater', text='Continue with Update After Viewing Changes?', ok_label='Yes', cancel_label='No'): return
			show_after_action = False
	if action == 1: kodi_utils.notification('Red Light Update Occuring', icon=kodi_utils.get_icon('downloads'))
	elif action == 2: return kodi_utils.notification('Red Light Update Available', icon=kodi_utils.get_icon('downloads'))
...
def rollback_check():
	...
	kodi_utils.hide_busy_dialog()
	if results.status_code != 200: return kodi_utils.ok_dialog(heading='Red Light Updater', text='Error rolling back.[CR]Please install rollback manually')
	results = results.json()
	results = [i['name'].split('-')[1].replace('.zip', '') for i in results if 'plugin.video.redlight' in i['name'] \
				and not i['name'].split('-')[1].replace('.zip', '') == current_version]
	if not results: return kodi_utils.ok_dialog(heading='Red Light Updater', text='No previous versions found.[CR]Please install rollback manually')
	...
	if not kodi_utils.confirm_dialog(heading='Red Light Updater',
...
def update_addon(new_version, action, show_after_action=True):
	...
	kodi_utils.notification('Red Light Performing Rollback' if action == 5 else 'Red Light Performing Update', icon=kodi_utils.get_icon('downloads'))
	zip_name = 'plugin.video.redlight-%s.zip' % new_version
	url = get_location('%s') % zip_name
	kodi_utils.show_busy_dialog()
	result = requests.get(url, stream=True)
	kodi_utils.hide_busy_dialog()
	if result.status_code != 200: return kodi_utils.ok_dialog(heading='Red Light Updater', text='Error Updating.[CR]Please install new update manually')
	...
	if not success: return kodi_utils.ok_dialog(heading='Red Light Updater', text='Error Updating.[CR]Please install new update manually')
	if action == 5:
		set_setting('update.action', '3')
		kodi_utils.ok_dialog(heading='Red Light Updater', text='[CR]Success.[CR]Red Light rolled back to version [B]%s[/B]' % new_version)
	elif action in (0, 4):
		if show_after_action:
			if kodi_utils.confirm_dialog(heading='Red Light Updater', text='[CR]Success.[CR]Red Light updated to version [B]%s[/B]' % new_version,
										ok_label='Changelog', cancel_label='Exit', default_control=10) != False:
				kodi_utils.show_text('Changelog', file=kodi_utils.translate_path('special://home/addons/plugin.video.redlight/resources/text/changelog.txt'), font_size='large')
		else:
